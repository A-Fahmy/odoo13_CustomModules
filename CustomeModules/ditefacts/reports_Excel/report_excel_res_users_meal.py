...
class PatientCardXLS(models.AbstractModel):
    _name = 'report.ditefacts.call_report_res_users_meal_excel'
    ...
    def generate_xlsx_report(self, workbook, data, line):
        ...
        for lines in line:
            ...
            sheet = workbook.add_worksheet('Res Meals Card %s' % (c))
            # sheet.right_to_left()
            sheet.set_column(3, 3, 50)
            sheet.set_column(2, 2, 30)
            sheet.write(2, 2, 'Name', format1)
            sheet.write(2, 3, lines.name, format2)
            sheet.write(3, 2, 'seq', format1)
            sheet.write(3, 3, lines.name_seq, format2)
